...
// --- Componente: Nota para Iniciantes ---
function BeginnerNote({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-now-green/5 border border-now-green/10 rounded-lg p-3 my-3">
      <p className="text-now-ivory/60 text-xs leading-relaxed">{children}</p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-now-ivory/70 text-sm mb-3">
          Primeiro, precisas de abrir o <strong className="text-now-green">terminal</strong> — é uma janela onde escreves comandos ao computador.
        </p>
        <BeginnerNote>
          <strong className="text-now-green">Como abrir o terminal:</strong><br />
          <strong>Mac:</strong> Carrega Cmd + Espaço, escreve &quot;Terminal&quot; e carrega Enter.<br />
          <strong>Windows:</strong> Carrega a tecla Windows, escreve &quot;PowerShell&quot; e carrega Enter.<br />
          <strong>Linux:</strong> Carrega Ctrl + Alt + T.<br /><br />
          Vais ver uma janela escura com um cursor a piscar. É aqui que vais colar os comandos. Para colar: <strong>Ctrl+V</strong> (Windows/Linux) ou <strong>Cmd+V</strong> (Mac). Depois carrega <strong>Enter</strong> para executar.
        </BeginnerNote>
        <p className="text-now-ivory/70 text-sm mb-3">
          Agora cola o comando para o teu sistema:
        </p>
...
function Module5Content({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <BeginnerNote>
        <strong className="text-now-green">Estes comandos só funcionam DENTRO do Claude Code.</strong> Primeiro abre o terminal, navega até ao teu projecto (<strong>cd o-meu-projecto</strong>), escreve <strong>claude</strong> e carrega Enter. Quando vires o cursor do Claude Code a piscar, aí sim, escreve estes comandos.<br /><br />
        A barra <strong>/</strong> antes do comando é obrigatória — é o que diz ao Claude &quot;isto é uma instrução especial, não uma pergunta normal&quot;.
      </BeginnerNote>
...
function Module6Content({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div><h3 className="text-now-green font-mono font-bold text-sm mb-2">PASSO 1 — Criar o CLAUDE.md</h3><BeginnerNote><strong className="text-now-green">Precisas de estar dentro do Claude Code</strong> na pasta do teu projecto. Se não tiveres o Claude Code aberto: abre o terminal, escreve <strong>cd o-meu-projecto</strong> e depois <strong>claude</strong>. Quando vires o cursor a piscar, estás pronto.</BeginnerNote><p className="text-now-ivory/70 text-sm mb-3">No Claude Code, escreve um destes comandos:</p><TerminalBlock command="/init" description="O Claude analisa o projecto e cria o CLAUDE.md por ti" /><p className="text-now-ivory/50 text-xs mt-2">Ou manualmente:</p><TerminalBlock command="Cria um ficheiro CLAUDE.md com informação sobre mim e o meu projecto. O meu nome é [nome], estou a construir [projecto], e o meu nível técnico é iniciante." description="Prompt para CLAUDE.md personalizado" /></div>
...
function Module7Content({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border-2 border-now-green/30 bg-now-terminal p-5 space-y-3"><h3 className="text-now-green font-mono font-bold">O QUE É O AGORA OX?</h3><p className="text-now-ivory/70 text-sm">Agentes AI que trabalham dentro do Claude Code. O Marquês de Pombal cria o plano do produto. O Fernando Pessoa analisa o mercado. Tu dás as ordens.</p></div>
      <div><h3 className="text-now-green font-mono font-bold text-sm mb-2">PASSO 1 — Instalar os agentes</h3><BeginnerNote><strong className="text-now-green">Certifica-te que o Claude Code está aberto</strong> na pasta do teu projecto. Se não estiver: abre o terminal → <strong>cd o-meu-projecto</strong> → <strong>claude</strong> → Enter. O CLAUDE.md que criaste no módulo anterior já está lá — o Claude já sabe quem és.</BeginnerNote><p className="text-now-ivory/70 text-sm mb-3">Agora, dentro do Claude Code, cola este prompt:</p><TerminalBlock command="Instala os agentes Fernando Pessoa (Analyst) e Marquês de Pombal (PM) do Agora OX. Cria as pastas necessárias em .claude/commands/ com os ficheiros de configuração de cada agente." description="Instala os 2 agentes fundamentais" /></div>
      <div><h3 className="text-now-green font-mono font-bold text-sm mb-2">PASSO 2 — Verificar</h3><p className="text-now-ivory/70 text-sm mb-3">Confirma que estão operacionais:</p><TerminalBlock command="/marques-de-pombal" description="Activa o Marquês de Pombal (PM) — deve responder-te" /><TerminalBlock command="/fernando-pessoa" description="Activa o Fernando Pessoa (Analyst) — deve responder-te" /></div>
      ...
      <div className="border border-now-green/20 rounded-lg p-4 bg-now-terminal"><p className="text-now-green font-mono font-bold text-sm mb-3">✅ CHECKPOINT</p><div className="space-y-1"><Checkpoint text="Agentes instalados em .claude/commands/" checked={checks[0]} onToggle={() => toggle(0)} /><Checkpoint text="/marques-de-pombal activado e a responder" checked={checks[1]} onToggle={() => toggle(1)} /><Checkpoint text="/fernando-pessoa activado e a responder" checked={checks[2]} onToggle={() => toggle(2)} /></div></div>
      {allDone && <button onClick={onComplete} className="w-full py-4 bg-now-green text-now-obsidian font-mono font-bold text-lg rounded-lg hover:bg-now-green/90 transition-all animate-pulse">Próximo Módulo →</button>}
      {!allDone && <p className="text-center text-now-green/30 text-xs font-mono py-4">Completa o checkpoint para desbloquear o próximo módulo</p>}
    </div>
  );
...
function Module8Content({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border-2 border-now-green/30 bg-now-terminal p-5 space-y-3"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-now-green text-black font-mono font-bold text-lg">FP</span><div><h3 className="text-now-green font-mono font-bold">Fernando Pessoa</h3><p className="text-now-ivory/40 text-xs font-mono">O teu investigador</p></div></div><p className="text-now-ivory/70 text-sm">Analisa o mercado, encontra oportunidades, estuda a concorrência. Os teus olhos no terreno antes de tomares qualquer decisão.</p><div className="bg-now-obsidian rounded-lg p-3 font-mono text-xs mt-3"><p className="text-now-ivory/50">Pergunta-lhe:</p><p className="text-now-green mt-1">Analisa o mercado para [a tua missão]</p></div><TerminalBlock command="/fernando-pessoa" description="Activa o Fernando Pessoa (Analyst)" /></div>
      <div className="rounded-lg border-2 border-now-green/30 bg-now-terminal p-5 space-y-3"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-now-green text-black font-mono font-bold text-lg">MP</span><div><h3 className="text-now-green font-mono font-bold">Marquês de Pombal</h3><p className="text-now-ivory/40 text-xs font-mono">O teu gestor de produto</p></div></div><p className="text-now-ivory/70 text-sm">Faz as perguntas certas e cria o PRD da tua missão. O gestor que reconstrói — transforma a tua ideia vaga num plano concreto e profissional.</p><div className="bg-now-obsidian rounded-lg p-3 font-mono text-xs mt-3"><p className="text-now-ivory/50">Pergunta-lhe:</p><p className="text-now-green mt-1">Quero construir [a tua ideia]. Cria o PRD.</p></div><TerminalBlock command="/marques-de-pombal" description="Activa o Marquês de Pombal (PM)" /></div>
      <div className="bg-now-green/5 border border-now-green/10 rounded-lg p-4"><p className="text-now-green text-xs font-mono">💡 Usa o Fernando Pessoa ANTES do Marquês de Pombal. Primeiro percebe o mercado, depois cria o plano. Informação antes de decisão.</p></div>
      <div className="border border-now-green/20 rounded-lg p-4 bg-now-terminal"><p className="text-now-green font-mono font-bold text-sm mb-3">✅ CHECKPOINT</p><div className="space-y-1"><Checkpoint text="Percebo o que o Marquês de Pombal faz (cria o PRD)" checked={checks[0]} onToggle={() => toggle(0)} /><Checkpoint text="Percebo o que o Fernando Pessoa faz (investiga o mercado)" checked={checks[1]} onToggle={() => toggle(1)} /></div></div>
      {allDone && <button onClick={onComplete} className="w-full py-4 bg-now-green text-now-obsidian font-mono font-bold text-lg rounded-lg hover:bg-now-green/90 transition-all animate-pulse">Próximo Módulo →</button>}
      {!allDone && <p className="text-center text-now-green/30 text-xs font-mono py-4">Completa o checkpoint para desbloquear o próximo módulo</p>}
    </div>
  );
...
function Module9Content({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="border-l-2 border-now-green pl-4"><p className="text-now-green/60 text-xs font-mono uppercase tracking-wider mb-1">A MISSÃO FINAL</p><p className="text-now-ivory text-base">Tudo converge aqui. Activa o Marquês de Pombal, dá-lhe a missão, e sai com um PRD completo — o plano do teu produto.</p></div>
      <div className="bg-now-green/5 border border-now-green/10 rounded-lg p-4"><p className="text-now-green text-sm font-mono font-bold mb-2">ANTES DE COMEÇAR</p><p className="text-now-ivory/70 text-sm">Abre o teu Blueprint da Zona de Genialidade (Tab ZG). Lê a secção 9 — o teu mini-PRD. Essa é a base. Agora o Marquês de Pombal vai transformá-la num documento profissional.</p></div>
      <div><h3 className="text-now-green font-mono font-bold text-sm mb-2">PASSO 1 — Activar o Marquês de Pombal</h3><BeginnerNote><strong className="text-now-green">Abre o Claude Code</strong> na pasta do teu projecto (terminal → <strong>cd o-meu-projecto</strong> → <strong>claude</strong>). Quando o cursor estiver a piscar, escreve o comando abaixo. O <strong>/marques-de-pombal</strong> é um &quot;slash command&quot; — diz ao Claude para activar o agente gestor de produto. É como chamar um colega de equipa.</BeginnerNote><TerminalBlock command="/marques-de-pombal" description="Activa o Marquês de Pombal (PM)" /></div>
      ...
      <div><h3 className="text-now-green font-mono font-bold text-sm mb-2">PASSO 3 — Responder e receber o PRD</h3><p className="text-now-ivory/70 text-sm">O Marquês de Pombal vai fazer-te 5-10 perguntas. Responde com honestidade. Quando tiver informação suficiente, gera o PRD completo.</p></div>
      <div className="border border-now-green/20 rounded-lg p-4 bg-now-terminal"><p className="text-now-green font-mono font-bold text-sm mb-3">✅ CHECKPOINT</p><div className="space-y-1"><Checkpoint text="Marquês de Pombal activado e a fazer perguntas" checked={checks[0]} onToggle={() => toggle(0)} /><Checkpoint text="Respondi a todas as perguntas" checked={checks[1]} onToggle={() => toggle(1)} /><Checkpoint text="PRD completo gerado e guardado" checked={checks[2]} onToggle={() => toggle(2)} /></div></div>
